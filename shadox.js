...
client.on("message", async (message) => {
    ...
    if (command === "!etkinlik" && "860119403978817546" == message.author.id) {
...
client.on("message", async (message) => {
    const args = message.content.split(" ");
    const command = args.shift();
    if (command === "!ilişki" && "860119403978817546" == message.author.id) {
...
client.on("message", async (message) => {
    const args = message.content.split(" ");
    const command = args.shift();
    if (command === "!katılımcı" && "860119403978817546" == message.author.id) {
    let çk = new disbut.MessageButton().setStyle('red').setLabel('Çekiliş Katılımcısı').setID('iv')
    let ek = new disbut.MessageButton().setStyle('red').setLabel('Etkinlik Katılımcısı').setID('iy')
    let fi = new disbut.MessageButton().setStyle("red").setLabel('Film İzleyicisi').setID('sy')
...
client.on("message", async (message) => {
    const args = message.content.split(" ");
    const command = args.shift();
    if (command === "!oyun" && "860119403978817546" == message.author.id) {
